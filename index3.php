...
<head>
    ...
    <style id="enhanced-ux-theme">
        :root {
            --shell-bg: #eef4ff;
            --shell-panel: rgba(255, 255, 255, 0.78);
            --shell-panel-strong: rgba(255, 255, 255, 0.92);
            --shell-border: rgba(148, 163, 184, 0.22);
            --shell-shadow: 0 24px 65px rgba(15, 23, 42, 0.12);
            --shell-shadow-soft: 0 14px 32px rgba(15, 23, 42, 0.08);
            --shell-ring: 0 0 0 3px rgba(42, 157, 143, 0.16);
            --shell-radius: 28px;
            --shell-radius-lg: 34px;
            --shell-radius-sm: 18px;
            --shell-text: #0f172a;
            --shell-muted: #5f6f85;
            --shell-surface-dark: rgba(14, 25, 44, 0.88);
        }

        [data-theme='dark'] {
            --shell-bg: #08111f;
            --shell-panel: rgba(12, 19, 35, 0.78);
            --shell-panel-strong: rgba(12, 19, 35, 0.92);
            --shell-border: rgba(148, 163, 184, 0.12);
            --shell-shadow: 0 24px 65px rgba(2, 8, 23, 0.45);
            --shell-shadow-soft: 0 14px 32px rgba(2, 8, 23, 0.25);
            --shell-text: #e2e8f0;
            --shell-muted: #9aa9c0;
        }
        ...
        body {
            background:
                ...
                linear-gradient(180deg, rgba(248, 250, 252, 0.84), rgba(232, 240, 255, 0.94));
        ...
        [data-theme='dark'] body {
            background:
                ...
                radial-gradient(circle at top right, rgba(96, 165, 250, 0.13), transparent 24%),
                linear-gradient(180deg, rgba(6, 11, 21, 0.98), rgba(5, 12, 23, 0.98));
        }
        ...
        .dashboard-card::before {
            ...
            position: absolute;
            inset: 0;
            background: linear-gradient(135deg, rgba(255, 255, 255, 0.24), transparent 50%, rgba(42, 157, 143, 0.08));
            pointer-events: none;
        }

        aside.sidebar h2 {
            margin-bottom: 24px;
            border-bottom: 1px solid rgba(42, 157, 143, 0.22);
            padding-bottom: 14px;
            font-size: 1.45rem;
        ...
        [data-theme='dark'] aside.sidebar nav ul li a {
        ...
        aside.sidebar nav ul li a.active {
            ...
            color: #ffffff;
            background: linear-gradient(135deg, rgba(42, 157, 143, 0.96), rgba(33, 133, 121, 0.94));
        ...
        [data-theme='dark'] .global-search-bar input,
        [data-theme='dark'] .search-sort-controls input,
        [data-theme='dark'] .search-sort-controls select,
        ...
        [data-theme='dark'] .public-content,
        ...
        #online-shop-cart,
        #my-orders {
            animation: shellFadeUp 0.45s ease both;
        }
        ...
        .dashboard-card {
            ...
            background:
                ...
                radial-gradient(circle at top right, rgba(42, 157, 143, 0.15), transparent 45%);
            box-shadow: var(--shell-shadow);
        }

        [data-theme='dark'] .dashboard-card {
            background:
                linear-gradient(145deg, rgba(18, 25, 41, 0.92), rgba(18, 25, 41, 0.72)),
                radial-gradient(circle at top right, rgba(42, 157, 143, 0.16), transparent 45%);
        }
        ...
        .search-sort-controls .form-group:first-child,
        .report-filters .form-group:first-child {
            grid-column: span 4;
        }
        ...
        .data-table thead th {
            ...
            letter-spacing: 0.08em;
            border-bottom: none;
            background: linear-gradient(135deg, rgba(42, 157, 143, 0.96), rgba(33, 133, 121, 0.96));
        }
        ...
        .public-header .login-btn {
            border-radius: 16px;
            padding: 11px 18px;
            box-shadow: 0 16px 28px rgba(244, 162, 97, 0.18);
        }
        ...
        .hero-section {
            ...
            background:
                ...
                linear-gradient(135deg, rgba(16, 33, 61, 0.98), rgba(42, 157, 143, 0.94));
            box-shadow: var(--shell-shadow);
        }
        ...
        .contact-info-box {
            background: linear-gradient(160deg, rgba(42, 157, 143, 0.96), rgba(17, 71, 115, 0.94));
            box-shadow: var(--shell-shadow);
        }
        ...
        .yasin-signature-footer a {
            color: #7dd3fc;
            text-decoration: none;
            font-weight: 600;
        }

        .yasin-signature-footer a:hover {
            text-decoration: underline;
        }
...
<body>
    ...
    <script id="enhanced-ux-script">
        (function () {
            ...
            function syncSidebarState(forceClose) {
                const sidebar = document.querySelector('aside.sidebar');
                if (!sidebar) return;
                if (forceClose) {
                    sidebar.classList.remove('active');
                }
                document.body.classList.toggle('sidebar-open', window.innerWidth <= 900 && sidebar.classList.contains('active'));
            }

            function injectSidebarBackdrop() {
                if (document.querySelector('.sidebar-backdrop') || !document.querySelector('aside.sidebar')) return;
                const backdrop = document.createElement('div');
                backdrop.className = 'sidebar-backdrop';
                backdrop.addEventListener('click', () => syncSidebarState(true));
                document.body.appendChild(backdrop);
            }

            function injectSignatureFooter() {
                if (document.querySelector('.yasin-signature-footer')) return;
                const footer = document.createElement('div');
                footer.className = 'yasin-signature-footer';
                footer.innerHTML = 'Created by: Yasin Ullah – Bannu Software Solutions <span>•</span> <a href="https://www.yasinbss.com" target="_blank" rel="noopener noreferrer">Website: https://www.yasinbss.com</a> <span>•</span> <span>WhatsApp: 03361593533</span>';
                document.body.appendChild(footer);
            }

            function enhancePublicContactForm() {
            ...
            function addSurfaceClassNames() {
                document.querySelectorAll('.card, .dashboard-card, .book-card, .login-card, .modal-content, .mv-card').forEach((item, index) => {
                    item.style.setProperty('--stagger', String(index % 12));
                });
            }
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                enhancePublicContactForm();
                addSurfaceClassNames();
                attachResponsiveLabels(document);
                syncSidebarState(false);

                const observer = new MutationObserver(() => attachResponsiveLabels(document));
                observer.observe(document.body, { childList: true, subtree: true });

                const hamburger = document.getElementById('hamburger-menu');
                if (hamburger) {
                    hamburger.addEventListener('click', function () {
                        setTimeout(() => syncSidebarState(false), 0);
                    });
                }
